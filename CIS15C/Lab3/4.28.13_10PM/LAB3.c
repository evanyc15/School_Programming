...
/*2*/void storeFile(hashTable arrayHash[],linkedData linkArray[])
{
	...
	//fscanf(fp,"%s\n",tempStr);  NEED TO <EXCLUDE> APOSTROPHES IN SCAN LATER
		...
		if(arrayHash[index].elemStatus == 1){
			...
			///*Testing*/ printf("arrayHash no collision\n");
...
{
	FILE*fp;
	userStrings inputStr[50];
	char filename[30];
	char tempStr[30];
	int strLength;
	int strIndex, keyIndex, i;
	int inQuit = 0;
	int notFound = 0;
	...
	while(fgets(tempStr,30,fp) != NULL){ //Not sure if any of this is correct -VVVVV
		strIndex = 0;
		inputStr[strIndex].inputWord = strtok (tempStr," ");
		while(tempStr != NULL){
			inputStr[strIndex].inputWord = strtok(tempStr," ");
			strIndex++;
		}
		for(i=0;i<strIndex;i++){
			strLength = strlen(inputStr[i].inputWord);
			keyIndex = (int)hash_Function((void*)(inputStr[i].inputWord),strLength);
			printf("Words not found:\n");
			if(arrayHash[keyIndex].elemStatus == 0){
				printf("%s",inputStr[i].inputWord);
				notFound ++;
			}
		}
		printf("Total number of words not found in dictionary: %d",notFound);
	}
}
