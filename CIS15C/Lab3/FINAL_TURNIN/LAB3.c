...
typedef struct linkedData{ //This structure is used to store the words for the linked lists before addNode
	char*linkWord;
}linkedData;
...
Hashing Algorithms used: Adding, Rotation, Modulo-Division
Reason for Algorithm: Using various Hashing algorithms has lowered the
chance of clustering. We first add the value of the chars (key) to a temporary unsigned
variable and then shift the number left and right and add or use an XOR to further
randomize the number. Afer all chars have been looped through, we then shift the
resulting number left and right again and add or use an XOR to even further
randomize the number. This significantly decreases the chance of the same index 
being used again. This also decrease the numbers of probes because less collisions 
will be called. Due to all the adding, the number becomes very large so in order to
counter this, a modulo division of 10000 is applied to greatly decrease the value's
size so that the array size can be smaller. When the program is run, the statistics
show the efficiency of this algorithm because all index values are kept within 5000 
and there are very few linked lists. These linked lists are also very short which
show there aren't many collisions especially at the same indexes.
...
/*4*/int compareWord(void* inHashWord1, void* inHashWord2)
{
	...
	word1 = (char*)inHashWord1;
	word2 = (char*)inHashWord2;
...
/*6*/void userInteraction(hashTable arrayHash[])
{
	...
	char*linkedWord;
	int strLength;
	int strCompare,equalStatus,from,current,next,i;
	unsigned index;
	int inQuit = 0;
	int notFound = 0;
	...
	{	
		...
		index = hash_Function((void*)tempStr,strLength);
		equalStatus = 0;
		if(arrayHash[index].word != NULL){
			if(arrayHash[index].collision == NULL){
				strCompare = strcmp(arrayHash[index].word,tempStr);
				if(strCompare == 0){
					equalStatus = 1;
				}
			}
			else if(arrayHash[index].collision != NULL){
				strCompare = strcmp(arrayHash[index].word,tempStr);
				if(strCompare == 0){
					equalStatus = 1;
				}
				from = 0;
				while((traverse(arrayHash[index].collision,from,(void**)&linkedWord)) != 0){
	...
	printf("\nTotal number of words not found in dictionary: %d\n",notFound);
	fclose(fp);
}//end userInteraction
