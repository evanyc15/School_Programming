...
/*9*/void sortInsertion(int randomArray[],int left,int right,int*compares,int*moves);
/*10*/void makeNearSort(int randomArray[]);
/*11*/void copyArray(int randomArray[],int copiedArray[]);

int main(void)
{
	int randomArray[MAX_ARRAY];
    int copiedArray[MAX_ARRAY];
	int randomNum,i;
    int* compares;
    int* moves;
    int comNum = 0;
    ...
    *compares = 0;
    *moves = 0;
    copyArray(randomArray,copiedArray);
    sortHeap(copiedArray,MAX_ARRAY-1,compares,moves);
	if(MAX_ARRAY == 100)
        printArray(copiedArray);
    printf("Array Size: %d\n",MAX_ARRAY);
    printf("random list heapSort:  compares = %d  moves = %d\n",*compares,*moves);
    makeNearSort(copiedArray);
    *compares = 0;
    *moves = 0;
    sortHeap(copiedArray,MAX_ARRAY-1,compares,moves);
    ...
    *moves = 0;
    copyArray(randomArray,copiedArray);
    sortShell(copiedArray,MAX_ARRAY-1,compares,moves);
	if(MAX_ARRAY == 100)
        printArray(copiedArray);
    printf("Array Size: %d\n",MAX_ARRAY);
    ...
    *moves = 0;
    copyArray(randomArray,copiedArray);
    sortQuick(copiedArray,0,MAX_ARRAY-1,compares,moves);
    if(MAX_ARRAY == 100)
        printArray(copiedArray);
    printf("Array Size: %d\n",MAX_ARRAY);
    ...
    *moves = 0;
    ...
    printf("The most efficient sort for a large, random list is: quick Sort\n"); 
    printf("The most efficient sort for a large, nearly ordered list is: quick Sort\n");
    printf("The most efficient sort for a small, random list is: quick Sort\n");
    printf("The most efficient sort for a small, nearly ordered list is: quick Sort\n");
...
/*3*/void sortQuick(int randomArray[],int left,int right,int*compares,int*moves)
{
	...
	if((right - left) >= 16){
        ...
        *moves+=1;
		...
		sortInsertion(randomArray,left,right,compares,moves);
}//end sortQuick
...
/*sortInsertion: This function is called by the quick Sort
 ...
 Input: randomArray(array of ints),left(int),right(int),compares(int*),moves(int*)
 Return: Nothing*/
/*9*/void sortInsertion(int randomArray[],int left,int right,int*compares,int*moves) //not sure if this works
{
	int hold;
	int walker;
	int current;
...
/*copyArray: This function copies the randomized array
 into another array so that the array will always be the
 same randomized array when passed into a sorting function
 Input: randomArray(array of ints), copiedArray(array of ints)
 Return: Nothing*/
/*11*/void copyArray(int randomArray[],int copiedArray[])
{
    int i;
